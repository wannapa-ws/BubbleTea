...
//        xAxis.setCenterAxisLabels(false)
        ...
        xAxis.setValueFormatter(object : ValueFormatter() {
            ...
            override fun getFormattedValue(value: Float): String? {
                ...
                if( value < 0 || value >= listBubbleTea.size ){
                    return ""
                }
                else{
                    return listBubbleTea[value.toInt()].name
                }
...
//        rightAxis.axisMinimum = 0f // this replaces setStartAtZero(true)
    ...
    fun Pie_Chart( chart: PieChart){
        ...
        //สุ่มข้อมูล 6 อัน
        val listBubbleTea = BubbleTea.getSampleBubbleTeaData(6)

        val entries: ArrayList<PieEntry> = ArrayList()
        for (bubbletea in listBubbleTea) {
            entries.add(PieEntry(bubbletea.amount, bubbletea.name))
        }

        val dataset = PieDataSet(entries, "Type of Bubble Tea")
...
//        dataset.setXValuePosition(PieDataSet.ValuePosition.OUTSIDE_SLICE)
        ...
        chart.setCenterText("Bubble Tea");
        chart.setCenterTextSize(7F)
    ...
    fun Line_Chart(chart : LineChart) {
        ...
        val dataSet = LineDataSet(entries, "Sales Statistics")
